...
exports.getPurchaseAmtDetails = (req, res) => {
  const { zsDelCode } = req.params;

  const purchaseAmtQuery = `
    ...
    GroupedData AS (
      SELECT
        ...
        CASE 
          ...
          ELSE mg.zsgrp_des 
        END AS materialName,
        SUM(tsm.zdamount) AS totalAmount
    ...
    SELECT
      ...
      COALESCE(SUM(CASE WHEN gd.zsmonth = (SELECT zsmonth FROM LatestMonths LIMIT 1) THEN 
        CASE WHEN gd.totalAmount < 0 THEN 0 ELSE gd.totalAmount END END), 0) AS latestMonthAmount,
      COALESCE(SUM(CASE WHEN gd.zsmonth = (SELECT zsmonth FROM LatestMonths LIMIT 1 OFFSET 1) THEN 
        CASE WHEN gd.totalAmount < 0 THEN 0 ELSE gd.totalAmount END END), 0) AS secondLatestMonthAmount,
      COALESCE(SUM(CASE WHEN gd.zsmonth = (SELECT zsmonth FROM LatestMonths LIMIT 1 OFFSET 2) THEN 
        CASE WHEN gd.totalAmount < 0 THEN 0 ELSE gd.totalAmount END END), 0) AS thirdLatestMonthAmount
    FROM GroupedData gd
  ...
  db.query(purchaseAmtQuery, [zsDelCode, zsDelCode, ...allowedGroups], (err, results) => {
    if (err) {
    ...
    res.status(200).json({
      message: 'Purchase amount details fetched successfully',
      data: results,
    });
  });
};

exports.getPurchaseAmtBySearch = (req, res) => {
  ...
  db.query(purchaseAmtQuery, [zsDelCode, ...allowedGroups, currentMonth, previousMonthFormatted, thirdLatestMonthFormatted], (err, results) => {
    if (err) {
      console.error('Database error:', err);
      return res.status(500).json({ message: 'Server error', error: err.message });
    }
