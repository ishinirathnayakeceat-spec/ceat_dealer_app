import 'package:ceat_dealer_portal/services/target_year_service.dart';
import 'package:flutter/material.dart';
import 'package:shared_preferences/shared_preferences.dart';
import 'curved_bar.dart';

class TargetYearCard extends StatefulWidget {
  const TargetYearCard({Key? key}) : super(key: key);

  @override
  State<TargetYearCard> createState() => _TargetYearCardState();
}

class _TargetYearCardState extends State<TargetYearCard> {

  final TargetYearService _targetYearService = TargetYearService();

  bool _isLoading = true;
  String _errorMessage = '';
  List<Map<String, dynamic>> _targetYearData = [];
  OverlayEntry? _overlayEntry;

  @override
  void initState() {
    super.initState();
    _fetchTargetYearData();
  }

  Future<void> _fetchTargetYearData() async {
    try {
      ...
      final response = await _targetYearService.getMaterialGroupPerformanceYear(dealerCode);
     ...
     if (response['data'] != null && response['data'] is List) {
      ...
      if (!mounted) return;
      setState(() {
        _errorMessage = 'Error fetching target-year data: $e';
        _isLoading = false;
      });
    }
  }
...
void _removeOverlay() {
  ...
  Widget build(BuildContext context) {
    ...
    final double maxDataValue = _targetYearData.isNotEmpty
        ? _targetYearData
            .map((item) => [
