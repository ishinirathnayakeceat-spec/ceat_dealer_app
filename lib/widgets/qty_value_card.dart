...
class QtyValueCard extends StatefulWidget {
  ...
  @override
  State<QtyValueCard> createState() => _QtyValueCardState();
}

class _QtyValueCardState extends State<QtyValueCard> {
  ...
  final PurchaseQtyService _purchaseQtyService = PurchaseQtyService();
  bool _isLoading = true;
  String _errorMessage = '';
  List<Map<String, dynamic>> _purchaseQtyData = [];
  ...
  Future<void> _fetchPurchaseData() async {
    if (widget.selectedYear.isNotEmpty && widget.selectedMonth.isNotEmpty) {
      await _fetchPurchaseQtyDatabySearch(widget.selectedYear,
                              (months.indexOf(widget.selectedMonth) + 1).toString(),);
    } else {
      await _fetchPurchaseQtyData();
    }
  }


  Future<void> _fetchPurchaseQtyData() async {
    try {
      ...
      final response = await _purchaseQtyService.getPurchaseQty(dealerCode);
      ...
      if (response['data'] != null && response['data'] is List) {
        List<Map<String, dynamic>> filteredData = List<Map<String, dynamic>>.from(response['data'])
            .where((item) =>
                _parseToDouble(item['latestMonthQuantity']) > 0 ||
                _parseToDouble(item['secondLatestMonthQuantity']) > 0 ||
                _parseToDouble(item['thirdLatestMonthQuantity']) > 0)
            .toList();

        setState(() {
          _purchaseQtyData = filteredData;
          _isLoading = false;
  ...
  Future<void> _fetchPurchaseQtyDatabySearch(String year, String month) async {
    ...
    try {
      ...
      final response = await _purchaseQtyService.getPurchaseQtyBySearch(
        dealerCode,
        year,
        month,
      );
  ...
  Widget build(BuildContext context) {
    ...
    final double maxDataValue = _purchaseQtyData.isNotEmpty
        ? _purchaseQtyData
            .map((item) => [
                  _parseToDouble(item['latestMonthQuantity']),
                  _parseToDouble(item['secondLatestMonthQuantity']),
                  _parseToDouble(item['thirdLatestMonthQuantity']),
                          ...
                          return GestureDetector(
                            ...
                            child: Padding(
                              ...
                              child: CurvedBar(
                                ...
                                value: _parseToDouble(item['latestMonthQuantity']),
                                comparisonValue: _parseToDouble(item['secondLatestMonthQuantity']),
                                secondComparisonValue: _parseToDouble(item['thirdLatestMonthQuantity']),
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            "$currentLabel: ${value.toStringAsFixed(2)} Qty",
            style: const TextStyle(
              fontSize: 14,
              fontWeight: FontWeight.bold,
              color: Colors.black,
          ...
          Text(
            "$previousLabel: ${comparisonValue.toStringAsFixed(2)} Qty",
            style: const TextStyle(
              fontSize: 14,
              fontWeight: FontWeight.bold,
              color: Colors.black,
